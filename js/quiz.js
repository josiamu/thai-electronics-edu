const QUIZ_CATEGORIES_TH = [
  ...
  { id:'thyristor', icon:'🔺', name:'ไทริสเตอร์ SCR/TRIAC', questions:[
    {q:'SCR ต่างจากไดโอดธรรมดาอย่างไร?',opts:['นำกระแสได้ทั้งสองทิศทาง','มีขาเกตไว้สั่งให้เริ่มนำกระแส','ไม่มีแรงดันตกคร่อมเลย','ใช้ได้กับไฟ DC เท่านั้น'],ans:1,exp:'SCR คือไดโอด 4 ชั้น (PNPN) ที่เพิ่มขาเกต (G) เข้ามา — ยังนำกระแสทิศเดียว A→K เหมือนไดโอด แต่จะเริ่มนำก็ต่อเมื่อถูกจุดชนวนที่เกต'},
    {q:'เมื่อ SCR ถูกจุดชนวนจนนำกระแสแล้ว ถ้าเอากระแสเกตออก จะเกิดอะไรขึ้น?',opts:['ดับทันที','ยังนำกระแสต่อไป (ล็อกตัว)','นำกระแสสลับทิศ','พังทันที'],ans:1,exp:'SCR ล็อกตัว (latch) ด้วยการป้อนกลับบวกภายใน — ทริกแวบเดียวก็นำกระแสค้างเอง ไม่ต้องจ่ายเกตต่อ'},
    {q:'วิธีที่ถูกต้องในการทำให้ SCR หยุดนำกระแสคือข้อใด?',opts:['ถอดกระแสที่ขาเกตออก','ป้อนแรงดันลบเข้าขาเกต','ทำให้กระแสที่ไหลผ่านลดต่ำกว่ากระแสโฮลดิ้ง I_H','เพิ่มแรงดันที่แอโนดให้สูงขึ้น'],ans:2,exp:'เกตสั่งให้ติดได้แต่สั่งให้ดับไม่ได้ ต้องลดกระแสให้ต่ำกว่า I_H เช่น ตัดวงจร ลัด A-K ชั่วขณะ หรือรอจุดตัดศูนย์ของไฟ AC'},
    {q:'กระแสโฮลดิ้ง (Holding current, I_H) หมายถึงอะไร?',opts:['กระแสสูงสุดที่ทนได้','กระแสต่ำสุดที่ยังคงสภาพนำกระแสไว้ได้','กระแสที่ขาเกตตอนจุดชนวน','กระแสรั่วตอนกั้นแรงดัน'],ans:1,exp:'I_H คือกระแสต่ำสุดที่ต้องไหลผ่านเพื่อให้ยังนำกระแสอยู่ ต่ำกว่านี้เมื่อไรก็ดับ — เป็นสาเหตุที่ดิมเมอร์เก่าทำหลอด LED กระพริบ เพราะโหลดกินกระแสน้อยเกินไป'},
    {q:'TRIAC ต่างจาก SCR อย่างไร?',opts:['นำกระแสได้สองทิศ จึงคุมไฟ AC ได้เต็มคลื่น','ไม่มีขาเกต','ใช้กับไฟ DC เท่านั้น','ทนกระแสได้น้อยกว่ามาก'],ans:0,exp:'TRIAC เหมือน SCR สองตัวต่อกลับทางกันโดยใช้เกตร่วม จึงนำกระแสได้ทั้งครึ่งบวกและครึ่งลบ ใช้ตัวเดียวคุมไฟ AC ได้เต็มคลื่น (ขาเรียก MT1/MT2/G)'},
    {q:'DIAC ทำหน้าที่อะไรในวงจรดิมเมอร์?',opts:['จำกัดกระแสให้หลอดไฟ','สร้างพัลส์จุดชนวนเมื่อแรงดันคร่อมถึง V_BO ≈ 30V','เรียงกระแสให้เป็น DC','กรองสัญญาณรบกวน'],ans:1,exp:'DIAC ไม่มีเกต จะแตกนำกระแสเองเมื่อแรงดันคร่อมถึง V_BO (≈30V) ทั้งสองทิศ จึงยิงพัลส์คมๆ เข้าเกต TRIAC ที่มุมเดียวกันทุกครึ่งคลื่น ทำให้ไฟไม่กระพริบ'},
    {q:'ในการควบคุมกำลังแบบมุมจุดชนวน (phase control) ถ้าเพิ่มมุม α ให้มากขึ้น ผลที่ได้คือ?',opts:['กำลังไฟที่โหลดเพิ่มขึ้น','กำลังไฟที่โหลดลดลง','กำลังไฟเท่าเดิมแต่ความถี่เปลี่ยน','แรงดันสูงขึ้นแต่กระแสลดลง'],ans:1,exp:'α คือระยะที่ชนวนช้าไปหลังจุดตัดศูนย์ ยิ่ง α มาก ช่วงนำกระแสยิ่งสั้น กำลังยิ่งลด (TRIAC ที่ α = 90° เหลือกำลังครึ่งหนึ่งพอดี)'},
    {q:'เลือก TRIAC ใช้กับไฟบ้าน 220V ควรเลือกพิกัดแรงดันเท่าใดเป็นอย่างน้อย?',opts:['50V','220V พอดี','400–600V ขึ้นไป','12V'],ans:2,exp:'ไฟบ้าน 220V rms มีค่ายอด ≈ 311V จึงต้องเผื่อพิกัดกั้นแรงดัน (V_DRM/V_RRM) ไว้ที่ 400–600V ขึ้นไปเพื่อความปลอดภัยจากสไปก์'},
  ]},
];
...
const QUIZ_CATEGORIES_EN = [
  ...
  { id:'thyristor', icon:'🔺', name:'Thyristors (SCR/TRIAC)', questions:[
    {q:'How does an SCR differ from an ordinary diode?',opts:['It conducts in both directions','It has a gate terminal that commands it to start conducting','It has no forward voltage drop at all','It works on DC only'],ans:1,exp:'An SCR is a four-layer (PNPN) diode with a gate (G) added. It still conducts one way, A to K, but only after the gate fires it.'},
    {q:'After an SCR has been fired into conduction, what happens if the gate current is removed?',opts:['It turns off immediately','It keeps conducting (it latches)','It starts conducting the other way','It fails instantly'],ans:1,exp:'Internal positive feedback latches the SCR on, so one brief trigger is enough — no continuous gate drive is needed.'},
    {q:'What is the correct way to turn an SCR off?',opts:['Remove the gate current','Apply a negative voltage to the gate','Reduce the current through it below the holding current I_H','Raise the anode voltage'],ans:2,exp:'The gate can turn it on but never off. The current must fall below I_H — break the circuit, briefly short A-K, or let the AC waveform cross zero.'},
    {q:'What does holding current (I_H) mean?',opts:['The maximum current it can carry','The minimum current needed to stay conducting','The gate current needed to fire it','The leakage current while blocking'],ans:1,exp:'I_H is the minimum current that must keep flowing for the device to stay on. Fall below it and it turns off — which is why old dimmers make LED bulbs flicker: the load draws too little current.'},
    {q:'How does a TRIAC differ from an SCR?',opts:['It conducts both ways, giving full-wave AC control','It has no gate','It works on DC only','It handles far less current'],ans:0,exp:'A TRIAC behaves like two SCRs back to back sharing one gate, so it conducts on both half cycles. One device gives full-wave AC control (terminals MT1/MT2/G).'},
    {q:'What does the DIAC do in a dimmer circuit?',opts:['Limits the lamp current','Produces the trigger pulse once the voltage across it reaches V_BO ≈ 30 V','Rectifies the AC to DC','Filters out noise'],ans:1,exp:'A DIAC has no gate — it breaks over on its own at V_BO (≈30 V) in either direction, firing a sharp pulse into the TRIAC gate at the same angle every half cycle, so the lamp does not flicker.'},
    {q:'In phase control, what happens as the firing angle α is increased?',opts:['Load power increases','Load power decreases','Power stays the same but the frequency changes','Voltage rises while current falls'],ans:1,exp:'α is how late the device fires after the zero crossing. Larger α means a shorter conduction window and less power (a TRIAC at α = 90° delivers exactly half power).'},
    {q:'For a 220 V mains load, what is the minimum sensible voltage rating for a TRIAC?',opts:['50 V','Exactly 220 V','400–600 V or more','12 V'],ans:2,exp:'220 V rms peaks at about 311 V, so the blocking rating (V_DRM/V_RRM) should be 400–600 V or higher to survive normal line spikes.'},
  ]},
];
