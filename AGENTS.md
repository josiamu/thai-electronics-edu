...
```
website/
├── css/style.css        — shared CSS ทุกหน้า (CSS custom properties, dark mode, bilingual + **`.bs-*` = ชุด CSS ของซิมทีละขั้น** (bs-wrap/bs-tabs/bs-tab/bs-canvas/bs-nav/bs-navbtn/bs-desc) ใช้ร่วมกันระหว่าง transistor.html กับ switching-psu.html — หน้าใหม่ที่ทำซิมทีละขั้นให้ใช้คลาสชุดนี้ได้เลย **ห้ามก๊อปไปฝังใน `<style>` ของหน้าตัวเอง**)
...
├── js/smps-steps.js    — Interactive step-by-step **ภาคจ่ายไฟสวิตชิ่ง LNK304** ในหน้า switching-psu.html (canvas #sp-canvas ใน wrap #smps-circuit, ใช้ CSS .bs-* ร่วมกับ bjt-circuit.js → query .bs-tab/.bs-desc **แบบ scope ในwrap ตัวเอง**; โทโพโลยี **buck สวิตช์ฝั่งสูง**: 310V bus → ขา D → มอสเฟตในไอซี → **ขา S = โหนดสวิตช์ (แกว่ง 0↔310V)** → L 1mH → +5V → โหลด → GND(=N), D2 คาโทดอยู่ที่ขา S = ทางไหลวนช่วง OFF, C ขา BP คร่อม BP–S, ตัวแบ่ง R4.3K/D3/R2K **กลับลงที่ขา S ไม่ใช่กราวด์** (ไอซีอ่าน FB เฉพาะช่วง OFF ที่ D2 ตรึง S ไว้ใกล้กราวด์ · D3 กันขา FB ติดลบตอน ON) — **อย่าวาดขา S ลงกราวด์เด็ดขาด** จะผิดทั้งวงจรและผิดบทเรียนความปลอดภัย; 8 ขั้น: 1 NTC กันไฟกระชาก → 2 D1 เรียงครึ่งคลื่น → 3 Pi-filter 310V → 4 จังหวะ ON (สะสมพลังงานใน L) → 5 จังหวะ OFF (ไหลวนผ่าน D2) → 6 ป้อนกลับที่ขา FB แบบ **ข้ามพัลส์ (ON/OFF control ไม่ใช่ PWM)** + ปุ่มโหลดเบา/หนัก → 7 ไม่แยกกราวด์ + ขา S ไม่ใช่กราวด์ → 8 ไล่วัด TP1–TP4; **ช่องเลือกอาการเสีย #sp-fault 6 แบบ** (ok/ntc/ics/cap/bp/d2 — ตาราง FAULTS คุมทั้งค่าที่จุดวัด สี ไฟไหล และคำอธิบาย ตรงกับคลินิกช่าง 4 เคสในหน้า); ป้ายจุดวัด TP1 หลัง D1 / TP2 บัส 310V / TP3 ขา S / TP4 ไฟ 5V + แถบสโคปล่าง canvas เปลี่ยนรูปคลื่นตามขั้น (ขั้น 1–3,7 แกน ms · 4–6 แกน µs ซูม ×1000); จุดที่วิ่งคือ **กระแสสมมติ (+→−)**)
...
├── switching-psu.html  — งานซ่อม: ภาคจ่ายไฟสวิตชิ่ง LNK304 บนแผงพัดลมดิจิทัล (CURRENT_PAGE='repair-smps') — ทำไมเลิกใช้ C-Drop (ตารางเทียบ), บล็อกไดอะแกรม SVG 5 ภาค (NTC 12D-9+D1 → Pi-filter C4.7µF/L1mH/C1µF = 310V → LNK304 (D/S/BP 0.1µF/FB) → บัค L1mH+D2+ZD = 5V → ป้อนกลับ R4.3K/R2K/D3 กลับขา FB), เจาะลึกรายภาค, คลินิกอาการเสีย 4 เคส (NTC/D1 ไม่มี 310V, LNK304 ช็อต D-S เปลี่ยนคู่ D1, คาปาฟิลเตอร์ค่าแห้ง→ติดๆ ดับๆ, C ขา BP หรือ D2 เสีย), **กฎ Non-Isolated** (N ของไฟบ้านเป็น GND ของ 5V → ห้ามจับบอร์ดขณะเสียบปลั๊ก, ห้ามคีบสโคปตรงๆ ถ้าไม่ผ่าน isolation transformer, **ห้ามหนีบกราวด์สโคปที่ขา S** เพราะเป็นโหนดสวิตช์ไม่ใช่กราวด์) + Interactive step-by-step 8 ขั้น + ช่องเลือกอาการเสีย 6 แบบ (js/smps-steps.js)
...
```
CURRENT_PAGE ids: `home`, `electricity`, `ohm`, `resistor`, `diode`, `transistor`, `thyristor`, `ic`, `timer555`, `opamp`, `logic`, `relay`, `opto`, `capacitor`, `inductor`, `power-supply`, `multimeter`, `soldering`, `home-wiring`, `oscilloscope`, `signal-generator`, `repair-fan`, `repair-stove`, `repair-smps`, `simulation`, `breadboard`, `relay-lab`, `formulas`, `tools`, `quiz`, `downloads`
...
### Navigation (nav.js)
- NAV_LESSON_GROUPS → mega menu "บทเรียน / Lessons" แบ่ง 5 กลุ่ม: พื้นฐานไฟฟ้า, อุปกรณ์อิเล็กทรอนิกส์, วงจรรวมและดิจิทัล (ic/timer555/opamp/logic), เครื่องมือวัดและทดสอบ, งานซ่อมเครื่องใช้ไฟฟ้า (fan-motor/induction-stove/switching-psu — กลุ่มที่โตได้เรื่อยๆ ตามเครื่องใช้ไฟฟ้าที่เพิ่ม)
...
| 📚 งานซ่อมเครื่องใช้ไฟฟ้า | fan-motor, induction-stove, switching-psu | `topic-card-blue` |
